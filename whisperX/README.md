...
Install the optional web UI dependencies and launch the local interface:

  pip install -e ".[webui]"
  whisperx-webui

The page lets you upload an audio or video file, choose the Whisper model, select CUDA or CPU, configure compute type and batch size, and download the generated transcript and subtitle files. On a CUDA system, the default settings use `cuda` with `float16`.
