...
app = FastAPI(title="Ollama Admin GPU Agent", version="1.3.0")
...
def query_system() -> dict:
    """Query host CPU usage and system RAM using psutil."""
...
@app.get("/system")
def get_system_info():
    """Return host CPU and RAM metrics as JSON."""
    try:
        return JSONResponse(content=query_system())
    except Exception as e:
        return JSONResponse(
            status_code=500,
            content={"error": str(e)},
...
@app.get("/gpu")
def get_gpu_info(index: int | None = Query(default=None, ge=0)):
    """Return GPU metrics as a JSON array.

    When `index` is given, only that GPU (0-based, in detection order) is
    returned — useful when one agent's container can see every GPU on the
    host (e.g. Docker Desktop's WSL2 GPU passthrough doesn't isolate
    individual devices) but should only report the one a given server uses.
    """
    ...
    try:
        ...
        if index is not None:
            if index >= len(gpus):
                return JSONResponse(
                    status_code=404,
                    content={"error": f"No GPU at index {index} ({len(gpus)} detected)"},
                )
            gpus = [gpus[index]]
