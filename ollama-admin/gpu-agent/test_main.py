...
class TestGpuEndpoint:
    ...
    @patch("main.detect_backend", return_value="nvidia")
    @patch("main.query_nvidia")
    def test_filters_by_index(self, mock_query, _mock_backend):
        mock_query.return_value = [
            {"name": "RTX 4090", "memoryTotal": 1, "memoryUsed": 1, "memoryFree": 1,
             "temperature": 65, "utilization": 45},
            {"name": "RTX 3090", "memoryTotal": 1, "memoryUsed": 1, "memoryFree": 1,
             "temperature": 72, "utilization": 80},
        ]
        res = client.get("/gpu?index=1")
        assert res.status_code == 200
        data = res.json()
        assert len(data) == 1
        assert data[0]["name"] == "RTX 3090"

    @patch("main.detect_backend", return_value="nvidia")
    @patch("main.query_nvidia")
    def test_index_out_of_range(self, mock_query, _mock_backend):
        mock_query.return_value = [
            {"name": "RTX 4090", "memoryTotal": 1, "memoryUsed": 1, "memoryFree": 1,
             "temperature": 65, "utilization": 45},
        ]
        res = client.get("/gpu?index=5")
        assert res.status_code == 404


class TestQuerySystem:
    """Tests for host CPU/RAM queries."""

    @patch("main.psutil.virtual_memory")
    @patch("main.psutil.cpu_percent")
    def test_reports_cpu_and_memory(self, mock_cpu_percent, mock_virtual_memory):
        mock_cpu_percent.return_value = 42.3
        mock_virtual_memory.return_value = MagicMock(
            total=32 * GIB_TO_BYTES,
            available=20 * GIB_TO_BYTES,
        )

        info = query_system()

        assert info["cpuPercent"] == 42.3
        assert info["memTotal"] == 32 * GIB_TO_BYTES
        assert info["memFree"] == 20 * GIB_TO_BYTES
        assert info["memUsed"] == 12 * GIB_TO_BYTES
...
class TestSystemEndpoint:
    ...
    @patch("main.query_system")
    def test_returns_system_data(self, mock_query):
        mock_query.return_value = {
            "cpuPercent": 15.5,
            "memTotal": 32 * GIB_TO_BYTES,
            "memUsed": 12 * GIB_TO_BYTES,
            "memFree": 20 * GIB_TO_BYTES,
        }
        res = client.get("/system")
        assert res.status_code == 200
        data = res.json()
        assert data["cpuPercent"] == 15.5
        assert data["memTotal"] == 32 * GIB_TO_BYTES

    @patch("main.query_system", side_effect=RuntimeError("psutil failure"))
    def test_handles_query_failure(self, _mock):
        res = client.get("/system")
        assert res.status_code == 500
        assert "psutil failure" in res.json()["error"]
